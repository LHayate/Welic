...
namespace Welic.WebSite.Utilities
{
    public class ImageHelper
    {
        ...
        {
            var filePath = string.Format("~/Imagens/images/listing/{0}.jpg", id.ToString(fileFormat));
        ...
        {
            var filePath = string.Format("~/Imagens/images/profile/{0}.jpg", name);
            if (File.Exists(HostingEnvironment.MapPath(filePath)))
            {
                return ImageVersion(filePath);
            }
        ...
        {
            var filePath = string.Format("~/Imagens/images/community/{0}.{1}", name, format);
            if (File.Exists(HostingEnvironment.MapPath(filePath)))
            {
                return ImageVersion(filePath);
            }
