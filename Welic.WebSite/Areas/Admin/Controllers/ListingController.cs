...
namespace Welic.WebSite.Areas.Admin.Controllers
{
    ...
    public class ListingController : Controller
    {
        ...
        public async Task<ActionResult> CategoryUpdate(CategoryModel model)
        {
            ...
            {
                model.Category.ObjectState = ObjectState.Added;
            ...
            else
            {
                ...
                categoryExisting.ObjectState = ObjectState.Modified;
        ...
        public async Task<ActionResult> CustomFieldUpdate(MetaField metaField, List<int> Categories)
        {
            if (metaField.ID == 0)
            {
                metaField.ObjectState =ObjectState.Added;
            ...
            else
            {
                ...
                metaFieldExisting.ObjectState = ObjectState.Modified;
        ...
        public async Task<ActionResult> ListingUpdate(Listing listing, FormCollection form, IEnumerable<HttpPostedFileBase> files)
        {
            ...
            if (files != null && files.Count() > 0)
            {
                foreach (HttpPostedFileBase file in files)
                {
                    if ((file != null) && (file.ContentLength > 0) && !string.IsNullOrEmpty(file.FileName))
                    {
                        ...
                        {
                            var path = Path.Combine(Server.MapPath("~/Imagens/images/listing"), string.Format("{0}.{1}", picture.ID.ToString("00000000"), "jpg"));
            ...
            if (updateCount)
            {
                //_sqlDbService.UpdateCategoryItemCount(listing.CategoryID); //TODO: Resolver
        ...
        public async Task<ActionResult> ListingPhotoDelete(int id)
        {
            try
            {
                ...
                var path = Path.Combine(Server.MapPath("~/Imagens/images/listing"), string.Format("{0}.{1}", id.ToString("00000000"), "jpg"));
        ...
        public async Task<ActionResult> ListingTypeUpdate(ListingType model)
        {
            ...
            else
            {
                ...
                listingTypeExisting.ObjectState = ObjectState.Modified;
