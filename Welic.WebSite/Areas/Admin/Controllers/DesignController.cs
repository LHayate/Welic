...
namespace Welic.WebSite.Areas.Admin.Controllers
{
    ...
    public class DesignController : Controller
    {
        ...
        public async Task<ActionResult> AppearanceUpdate(AppearanceModel model)
        {
            ...
            settingExisting.ObjectState = ObjectState.Modified;
        ...
        public void SavePicture(HttpPostedFileBase file, string name, Size size, string formatFile = "jpg")
        {
            if ((file != null) && (file.ContentLength > 0) && !string.IsNullOrEmpty(file.FileName))
            {
                ...
                {
                    var path = Path.Combine(Server.MapPath("~/Imagens/images/community"), string.Format("{0}.{1}", name, formatFile));
        ...
        public async Task<ActionResult> PictureDelete(string key, string formatFile = "jpg")
        {
            try
            {
                var path = Path.Combine(Server.MapPath("~/Imagens/images/community"), string.Format("{0}.{1}", Path.GetFileName(key), formatFile));
