...
{
    public partial class FormBuscaPaginacao : FormWelic
    {
        ...
        public FormBuscaPaginacao()
        :this(null,null,false,false,null,null,null,null,null,30)
        { }
        ...
        public async void SetListAsync<T>(List<T> list, string pColunaPrincipal)
        {
            if (list == null || list.Count <= 0)
            {
                MessageBox.Show("Nenhum Dado encontrado", "Vazio", MessageBoxButtons.OK);
                return;
            }
                
            

            this._dtPesquisaFiltrada = this._dtPesquisaTotal = ConvertListToDataTable.ToDataTable<T>(list) ;
            PreencheComboColuna(pColunaPrincipal);

            PreencheCampoFiltro("");

            tsLabelTotalPaginas.Text = (Math.Ceiling(((double)_dtPesquisaFiltrada.Rows.Count / (double)this._QtdeRegistrosPagina))).ToString(CultureInfo.InvariantCulture);

            SelecionarPagina(1);

            AjustarTamanhoFormulario();
        }
        public void SetList<T>(List<T> list, string pColunaPrincipal)
        {
            if (list == null || list.Count <= 0)
            {
                MessageBox.Show("Nenhum Dado encontrado", "Vazio", MessageBoxButtons.OK);
        ...
        public T RetornoList<T>()
        {
            if(retorno != null)
                return ConvertDataTableToList.GetItem<T>(this.retorno);
        ...
        private void SelecionarPagina(int pPagina)
        {
            if (_dtPesquisaFiltrada != null && _dtPesquisaFiltrada.Rows.Count > 0)
            {
                ...
                {
                    dgvLista.DataSource = _dtPesquisaTotal;
                    _Pagina = pPagina;
                }
            }
            else
        ...
        private void cboColuna_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (cboColuna.SelectedValue.ToString())
            {
                case ("System.Decimal"):
                    ...
                    txtFiltro.TipoValor = TextBoxWelic.CTipoValor.Numerico;
                    txtFiltroEntre1.TipoValor = TextBoxWelic.CTipoValor.Numerico;
                    txtFiltroEntre2.TipoValor = TextBoxWelic.CTipoValor.Numerico;
                ...
                case "System.String":
                    ...
                    txtFiltro.TipoValor = TextBoxWelic.CTipoValor.Geral;
                    txtFiltroEntre1.TipoValor = TextBoxWelic.CTipoValor.Geral;
                    txtFiltroEntre2.TipoValor = TextBoxWelic.CTipoValor.Geral;
