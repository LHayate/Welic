...
{
    public partial class frmAreasConcentracoesCursos : FormCadastro
    {
        public object area { get; set; }

        public frmAreasConcentracoesCursos()
        {
            InitializeComponent();
        ...
        private void LoadFormulario(object sender, EventArgs e)
        {
            ...
            if (area != null)
            {
                PreencherCampos(this.Controls, BLL.BuscarSgaAreasConcentracoesCursos(area));
            }
            ...
            else
            {
                if (MessageBox.Show("Confirma a exclusão do registro?", "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        //Busca pela chave
                        int area = int.Parse(txtResponsabilidade.Text);
                        c = md.AREAS.FirstOrDefault(w => w.AREA == area);
            ...
            else
            {
                try
                {
                    ...
                    if (AcaoFormulario == CAcaoFormulario.Novo)
                    {
                        ...
                        txtResponsabilidade.Text = md.AREAS.Max(x => x.AREA).ToString();
                    }
        ...
        private void ClickAuditoria(object sender, EventArgs e)
        {
            ...
            frmAuditoriaFormulario frmAuditoriaFormulario = new frmAuditoriaFormulario("AREAS", "SGA", "area", txtResponsabilidade.Text);
