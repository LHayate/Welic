...
{
    public partial class frmUnidadesCustosDiretos : FormCadastro
    {
        public object unidade { get; set; }
        public frmUnidadesCustosDiretos()
        {
            InitializeComponent();
        ...
        private void LoadFormulario(object sender, EventArgs e)
        {
            ...
            if (unidade != null)
            {
                PreencherCampos(this.Controls, BLL.BuscarSgaUnidadesCustosDiretos(unidade));
            }
            ...
            else
            {
                if (MessageBox.Show("Confirma a exclusão do registro?", "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        UNIDADES_CUSTOS_DIRETOS c;

                        //Busca pela chave
                        int unidade = int.Parse(txtUnidade.Text);
                        c = md.UNIDADES_CUSTOS_DIRETOS.FirstOrDefault(w => w.UNIDADE == unidade);
        ...
        private void ClickGravar(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        //Seta campos
                        c = new UNIDADES_CUSTOS_DIRETOS();

                        c.DESCRICAO = txtDescricao.Text.Trim();

                        //Insere Registro                    
                        md.AddToUNIDADES_CUSTOS_DIRETOS(c);
                    }
                    ...
                    if (AcaoFormulario == CAcaoFormulario.Novo)
                    {
                        ...
                        txtUnidade.Text = md.UNIDADES_CUSTOS_DIRETOS.Max(x => x.UNIDADE).ToString();
                    }
        ...
        private void ClickAuditoria(object sender, EventArgs e)
        {
            ...
            frmAuditoriaFormulario frmAuditoriaFormulario = new frmAuditoriaFormulario("UNIDADES_CUSTOS_DIRETOS", "SGA", "UNIDADE", txtUnidade.Text);
