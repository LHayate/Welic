...
{
    public partial class frmItensCustosDiretos : FormCadastro
    {
        public object item { get; set; }
        public frmItensCustosDiretos()
        {
            InitializeComponent();
        ...
        private void LoadFormulario(object sender, EventArgs e)
        {
            ...
            if (item != null )
            {
                PreencherCampos(this.Controls, BLL.BuscarSgaItensCustosDiretos(item));
            }
            ...
            else
            {
                if (MessageBox.Show("Confirma a exclusão do registro?", "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        ITENS_CUSTOS_DIRETOS c;

                        //Busca pela chave
                        int item = int.Parse(txtItem.Text);
                        c = md.ITENS_CUSTOS_DIRETOS.FirstOrDefault(w => w.ITEM == item);
        ...
        private void ClickGravar(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        //Seta campos
                        c = new ITENS_CUSTOS_DIRETOS();

                        c.DESCRICAO = txtDescricao.Text.Trim();

                        //Insere Registro                    
                        md.AddToITENS_CUSTOS_DIRETOS(c);
                    }
                    ...
                    if (AcaoFormulario == CAcaoFormulario.Novo)
                    {
                        ...
                        txtItem.Text = md.ITENS_CUSTOS_DIRETOS.Max(x => x.ITEM).ToString();
                    }
        ...
        private void ClickAuditoria(object sender, EventArgs e)
        {
            ...
            frmAuditoriaFormulario frmAuditoriaFormulario = new frmAuditoriaFormulario("ITENS_CUSTOS_DIRETOS", "SGA", "ITEM", txtItem.Text);
